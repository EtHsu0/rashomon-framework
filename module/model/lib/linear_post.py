...
The code is mostly from https://github.com/uiuctml/fair-classification/blob/main/postprocess.py
"""
...
class LinearPostProcessor:
  ...
  def fit(self,
    ...
    if solve_primal:
      ...
      # Try to solve with specified solver, fall back if it fails
      try:
        problem.solve(solver=solver, **solve_kwargs)
      except Exception as e:
        logger = logging.getLogger("LinearPostProcessor")
        logger.warning(f"Solver {solver} failed with error: {e}. Trying fallback solvers...")
        
        # Try fallback solvers
        for fallback_solver in [cp.ECOS, cp.SCS, cp.OSQP]:
          try:
            logger.info(f"Attempting fallback solver: {fallback_solver}")
            problem.solve(solver=fallback_solver)
            if problem.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
              logger.info(f"Fallback solver {fallback_solver} succeeded")
              break
          except Exception as fallback_e:
            logger.warning(f"Fallback solver {fallback_solver} also failed: {fallback_e}")
            continue
        else:
          # All solvers failed
          raise RuntimeError(f"All solvers failed. Original error: {e}")
