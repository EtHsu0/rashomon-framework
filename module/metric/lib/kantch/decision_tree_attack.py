from module.metric.lib.kantch.attack import AttackWrapper
...
class DecisionTreeAttackWrapper(AttackWrapper):
    ...
    def adversarial_examples(self, X, y, order, options={}):
        ...
        for sample, label in tqdm(zip(X, y), total=X.shape[0], disable=disable):
            ...
            curr_box = None
            for idx, (bounding_box, prediction) in enumerate(zip(bounding_boxes, predictions)):
                if not (np.any(sample <= bounding_box[:, 0]) or np.any(sample > bounding_box[:, 1])):
                    if curr_box is not None:
                        raise ValueError("Sample is in two bounding boxes, something is wrong")
                    curr_box = idx
        ...
        print(f"Adversarial examples generated: {success}/{total} ratio {success/total:.2f}, epsilon: {epsilon}")
